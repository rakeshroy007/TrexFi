...
export async function scanReceipt(file) {
    try {
        ...
        // ➡️ Convert File to ArrayBuffer
        const arrayBuffer = await file.arrayBuffer()

        // ➡️ Convert ArrayBuffer to Base64
        ...
        const text = response.text()        // It has like this format --> /```json djifjdijsjf json```/     so we need to clean this text...
        console.log("text is: ", text)
        const clenedText = text.replace(/```(?:json)?\n?/g, "").trim()
        
        try {
            const data = JSON.parse(clenedText) 
            console.log(data) 
            return {
...
export async function updateTransaction(id, data) {
    try {
        ...
        // ➡️ Update transaction and account balance in a transaction
        const transaction = await db.$transaction(async (tx) => {
        ...
        revalidatePath("/dashboard", 'page')
        revalidatePath(`/account/${data.accountId}`, 'page')
